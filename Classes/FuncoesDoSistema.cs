...
namespace CadastroFornecedoresGrupoSym
{
    class FuncoesDoSistema
    {
        ...
        public List<ClasseEmpresa> ListagemEmpresas(List<Empresa> Tabelaempresas) 
        {
            List<ClasseEmpresa> RetornoEmpresas = (from z in Tabelaempresas
                                   select new ClasseEmpresa { Value = z.ID, Names = z.NomeFantasia }).ToList();

            return RetornoEmpresas;
        }

        public List<ClassesDeListagem> FornecedoresAssociadosSQL(int EmpresaID)

        {
            ...
            List<ClassesDeListagem> ListagemFornecedoresSelecionados = (from z in SelectFornecedoresSelecionados
                                                       select new ClassesDeListagem { IdFornecedor = z.ID, NomeFornecedor = z.Nome }).ToList();
